namespace OpenAPI4Net.Examples
{
    ...
    public class Payunit
    {
        ...
        private const string SOURCE = ""; //"OpenAPI4Net.Examples.Payunit";
        ...
        public static void Test()
        {
            try
            {
                PayunitApi api = new PayunitApi();

                #region 获取单个资源
                _logger.Info("**** get ****");

                BusinessObject bo = api.Get("C00000001");
                ...
                _logger.Info(" 业务数据");
                if (bo.Body != null)
                    _logger.Debug(SOURCE, bo.Body.ToString());

                _logger.Info(" 业务数据.编码");
                if (bo.Full.IsArray(bo.ResourceId))
                {
                    if (bo.BodyArray != null && (bo.BodyArray.GetObject(0) != null))
                    {
                        _logger.Debug(SOURCE, bo.BodyObject["code"].ToString());
                        _logger.Debug(SOURCE, bo.BodyArray.GetObject(0).GetValue("code").ToString());
                    }
                }
                else
                {
                ...
                parameters.Add("code_begin", "C00000001");
                parameters.Add("code_end", "C00000003");
